...
class BUTTONS:
    def __init__(self):
        self.width, self.height = FIRST_PART.DATAS.const.WIDTH_BUTTON, FIRST_PART.DATAS.const.HEIGHT_BUTTON
        self.safe_new_inf = FIRST_PART.DATAS.write_new_datas
        self.action_for_locations = FIRST_PART.MECHANICA.Change_locations.LOCATIONS()

    def entity_button(self, pos, for_contact=False):
        if for_contact:
            self.width, self.height = FIRST_PART.DATAS.const.AREA_WIDTH, FIRST_PART.DATAS.const.AREA_HEIGHT
